================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: CArraySortByLambda プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した CArraySortByLambda には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには CArraySortByLambda アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

CArraySortByLambda.vcxproj
...
CArraySortByLambda.vcxproj.filters
...
CArraySortByLambda.h
   これはアプリケーションのメイン ヘッダー ファイルです。このファイルには、
   Resource.h を含む、その他のプロジェクト固有のヘッダーが含まれていて、
   CCArraySortByLambdaApp アプリケーション クラスを宣言します。

CArraySortByLambda.cpp
   これは、アプリケーション クラス CCArraySortByLambdaApp を含むメイン アプリケー
   ション ソース ファイルです。

CArraySortByLambda.rc
...
res\CArraySortByLambda.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル CArraySortByLambda.rc に含まれていま
   す。

res\CArraySortByLambda.rc2
...
CArraySortByLambdaDlg.h, CArraySortByLambdaDlg.cpp - ダイアログ
   これらのファイルは CCArraySortByLambdaDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な CArraySortByLambda.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   CArraySortByLambda.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
